...
################################################################################
# complete randomization of alpha matrix - unconstrained scenario
################################################################################
alpha_value <- c(alpha_matrix) # vectorize alpha coefficients 

sim_alpha_UN <- list()  # store all the generated random alpha matrices
diag_mean_UN <- c() # store mean of diagonal values
off_diag_mean_UN <- c() # store mean of off-diagonal values

for (i in 1:n_rand){
  set.seed(i)
  sim_alpha_UN[[i]] <- matrix(sample(alpha_value), ncol=8, nrow=8)
  sim_alpha <- sim_alpha_UN[[i]]
  diag_mean_UN[i] <- mean(diag(sim_alpha))
  value <- c(sim_alpha) # vectorize alpha values from the matrix
  off_diag <- value[!value %in% diag(sim_alpha)] # select non-diagonal values
  off_diag_mean_UN[i] <- mean(off_diag)
}


################################################################################
# diagonal + non-diagonal randomization without exchange between those two constrained scenario
################################################################################

j = row(alpha_matrix) != col(alpha_matrix) # get indices for non diagonal values
...
for (i in 1:n_rand){
  ...
  sim_alpha_CS[[i]] <- sim_alpha
  # fill in the diagonal with diagonal values
  diag(sim_alpha_CS[[i]]) <- sample(diag(alpha_matrix))
}


################################################################################
# simulate community level biomass growth (per plot) using interaction
##strength and relate community level biomass to richness 
################################################################################
...
# Create the dataframe structure the same as empirical data ###
...
### parameters for tree growth calculation (setup from BEF-China) ####
...
nsp <- unlist(sp_comp) |> unique() |> length() #pipe operator left hand side (LHS) of the operator as the first argument of the right hand side (RHS).
...
# - extract other mean parameters (2nd column!) from parameter overview (beta, theta, b)
vec_beta_sp <- pars[grep("^beta", row.names(pars)), 1]
theta <- pars["theta", 1]
b <- pars["b", 1]

# _ 1.1 unconstrained ####
print(length(sim_alpha_UN))

# _ 1.2 constrained ####
print(length(sim_alpha_CS))
...
sim_alpha_list <- c(list(no_inter, alpha_matrix), sim_alpha_UN, sim_alpha_CS)
scenario <- c("no_inter", "real_inter",
              rep("unconstrained", length(sim_alpha_UN)),
              rep("constrained", length(sim_alpha_CS)))

# test only one simulated alpha_matrix
scenario <- c("no_inter", "real_inter", "unconstrained",  "constrained")



## calculating growth and interaction effects
################################################################################
out_long <- c()
...
# - focus on last year
dat_main <- out_long[out_long$varname != "biomass" & 
                       out_long$year == 6, ] |>
  data_summary("mean", c("log.div", "alpha_sel", "scenario", "varname"))
dat_main[1] <- NULL # delete redundant varname column
...
# - add mean shuffled 
tmp1 <- dat_main[dat_main$alpha_sel > 2, ] |>
  data_summary("mean", c("log.div", "scenario", "varname"))
tmp1[1] <- NULL # delete redundant varname column
tmp1$alpha_sel <- 0 # assign 0 as alpha_sel value

dat_main <- rbind(dat_main, tmp1)
...
offdiag <- sim_alpha_list[2:(n_rand+2)] |>
  sapply(function(x) mean(x[row(x) != col(x)]))

diag <- sim_alpha_list[2:(n_rand+2)] |>
  sapply(function(x) mean(x[row(x) == col(x)]))
...
dat_summary <- rbind(
  data.frame(alpha_sel = 2:(n_rand+2),
             diff = offdiag - diag,
             slope = slope_growth[2:(n_rand+2)],
             response = rep("productivity", n_rand+1),
             col = c(2, rep(1, n_rand))),
  data.frame(alpha_sel = 2:(n_rand+2),
             diff = offdiag - diag,
             slope = slope_inter[2:(n_rand+2)],
             response = rep("net interactions", n_rand+1),
             col = c(2, rep(1, n_rand)))
)
...
p1 <- ggplot(dat_main, aes(log.div, mean, group = as.factor(alpha_sel))) +
  # geom_hline(yintercept = 0, lty = 2) +
  geom_line(data = dat_main[dat_main$alpha_sel > 2, ], linewidth = 0.2, color = "darkgrey") +
  
  # no inter:
  geom_line(data = dat_main[dat_main$alpha_sel == 1, ], color = "black", lty = 2) +
  # mean shuffled:
  geom_line(data = dat_main[dat_main$alpha_sel == 0, ], size = 1.8, color = "white") +
  geom_line(data = dat_main[dat_main$alpha_sel == 0, ], size = 1.2, color = "#023858") +
  # real - growth & interaction:
  geom_line(data = dat_main[dat_main$alpha_sel == 2, ], size = 1.8, color = "white") +
  geom_line(data = dat_main[dat_main$alpha_sel == 2, ], size = 1.2, color = "#8A9A5B") + 
  
  scale_x_continuous(breaks = c(0, 1, 2, 3), name = "Species Richness (log2)") +
  scale_y_continuous(name = "Community Mean Effect", 
                     breaks = c(-10, 0, 10, 20, 30, 2600, 2700, 2800, 2900, 3000)) +
  
  facet_grid2(varname~scenario, scale = "free", independent = "all") +
  labs(tag = "(a)") +
  theme_classic() +
...
p_out <- gridExtra::grid.arrange(grobs = list(p1, p2), nrow = 1)

ggsave( "set1_Fig3.png", plot = p_out, width = 173, height = 108, unit = "mm", dpi = 600)
