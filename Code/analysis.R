...
#### this script is for one data set, for the other the analysis is the same ###
################## but with different input stan_fit object  ###################
#------------------------------------------------------------------------------#
...
# extract the posteriors from the stan_fit object
post <- as.matrix(fit)

# extract the posteriors of alpha
alpha_post <- post[  ,grep("^alpha", colnames(post))]

# extract the estimated mean interaction coefficients
alpha <- pars[grep("^alpha", row.names(pars)), ]

# convert to matrix format
# row a1, column a3 denotes a1,3, meaning how species 3 affects species 1
alpha_matrix <- round(matrix(data = alpha[ , "mean"], nrow = 8, ncol = 8, 
                             byrow = TRUE), digits = 3)

rownames(alpha_matrix) <- c("a1", "a2", "a3", "a4", "a5", "a6", "a7", "a8")
colnames(alpha_matrix) <- c("a1", "a2", "a3", "a4", "a5", "a6", "a7", "a8")

# save the alpha matrix and parameter estimates that will be used in alpha_sim.R
save(alpha_matrix, pars, file = "interaction_set1.RData") 
...
p1 <- ggplot(alpha_df, aes(x = Var2, y = Var1)) + 
  ...
  scale_colour_gradient2(
    ...
    name = "alpha",
    aesthetics = "fill") +
  labs(x="", y="", title = "Dataset 2", tag = "(a)") +
...
# plot the network of alpha matrix
...
p2 <- qgraph(t(alpha_matrix), layout = "circle", 
       ...
       label.cex=1.3,
       label.color = "#636363",
       label.prop=0.8,  # proportion of the node and label
       border.color = "#636363",
       border.width = 3,
       posCol= col.pos,       
       negCol= col.neg,     
...
# plot alpha_i_j versus alpha_j_i
...
## p5 is for dataset 1
p5 <- ggplot() + 
     geom_density(aes(x=dif_1),linewidth=1.5, colour="#E96479")+
  ...
  annotate("text", x = mean(dif_1), y = 2, label = "100%", size = 3) +
  theme_classic() +
