...
parameters{
  ...
  real alpha;             // average interaction coefficients
...
model{
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
          interactions = interactions + alpha * (neib_bm[i,j] / focal_bm[i])^b;		  
        }
        // prediction
        predBM = focal_bm[i] + max({beta[fsp_ID[i]]*focal_bm[i]^theta + interactions, 0}) * max({1 + eps_p[fplot_ID[i]] + eps_spp[spp[i]] + eps_y[year_ID[i]] + eps_spy[spy[i]], 0}); 
